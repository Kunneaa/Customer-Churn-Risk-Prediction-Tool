...
from app.config import DEFAULT_CV_SPLITS, METRICS_FILE, MODEL_BENCHMARK_FILE, MODEL_FILE, TRAIN_FILE
from app.io_utils import ensure_parent_dir, save_json
from app.modeling import train_pipeline


def run_train(cv_splits: int | None = None, verbose: bool = True) -> dict[str, object]:
    if not TRAIN_FILE.exists():
        raise FileNotFoundError(f"Training file not found: {TRAIN_FILE}")

    train_df = pd.read_csv(TRAIN_FILE)
    effective_cv = cv_splits if cv_splits is not None else DEFAULT_CV_SPLITS
    if effective_cv < 2:
        raise ValueError("cv_splits must be >= 2")
    pipeline, metrics = train_pipeline(train_df, cv_splits=effective_cv, verbose=verbose)
